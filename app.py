...
st.markdown("""
    <style>
        ...
        div.stRadio > label.css-1ccx87l.e16fv1bt2:has(input:checked) { /* Targets the checked state, adjusted selector for current Streamlit versions */
        ...
        /* Remove the problematic CSS rule that hides the radio label completely,
           as we are now explicitly putting a subheader for navigation title. */
                    ...
                    if submit_ai_assisted_food:
                        if new_food_name_ai_assisted and new_food_calories_ai_assisted >= 0:
                            if new_food_name_ai_assisted not in st.session_state.food_database:
                                ...
                                st.success(f"已新增 '{new_food_name_ai_assisted}' (熱量: {new_food_calories_ai_assisted} 大卡) 已成功新增到食物資料庫！太棒了！")
                            else:
...
# 在 st.radio 上方加入一個美觀的導覽標題
st.subheader("💖 導覽您的健康日記 💖")

# st.radio 的 index 參數直接使用 session state 中的索引
# 為了隱藏 Streamlit 預設的 label，這裡將 label 設為空字串
selected_page_index_from_radio = st.radio(
    "", # 將導覽標籤設定為空字串，我們使用上方的 st.subheader 作為總標題
    options=page_names,
